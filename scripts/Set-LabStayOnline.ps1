...
$response = & $invoke `
    ...
    -Command "labstay $PlayerName $normalizedState"
$expected = "LAB stay-online protection for $PlayerName is now $($State.ToUpperInvariant())."

if (-not $response.Contains($expected)) {
    throw "LAB stay-online protection was not confirmed by the world server: $response"
}

Write-Output "LAB_STAY_ONLINE_$($State.ToUpperInvariant()):PLAYER=$PlayerName"
