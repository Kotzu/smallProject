(function(){
  ...
  function itemRow(x){
    const empty=!x.id,enchant=x.enchant?`<div class="armory-enchant ${x.enchantVerified?'':'pending'}">Enchant: ${esc(x.enchant)}</div>`:'<div class="armory-enchant muted">Enchant: —</div>',gem='<div class="armory-gem muted">Gem: — · Classic Era fără socket</div>';
  ...
  function renderOne(p){
    ...
    root.innerHTML=`<div class="armory-head"><div><h3>${esc(s.race)} ${esc(s.class)} · ${esc(s.spec)}</h3><span>${esc(s.gear)}</span></div><div class="audit-badge ${badge}">STAT AUDIT: ${audit.status}</div></div><div class="armory-meta"><b>Main scaling:</b> ${esc(model?.primary||'—')}<br><span>${esc(audit.reason)}</span>${audit.warning?`<br><span class="amber">${esc(audit.warning)}</span>`:''}</div><div class="audit-checks">${checks}</div><div class="armory-slots">${slots.map(itemRow).join('')}</div><h4>Character Stats</h4><div class="armory-stats">${stats.map(([k,v])=>`<div><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('')}</div><h4>Stat breakdown pe clasă</h4><div class="stat-breakdown">${(model?.lines||[]).map(x=>`<div><b>${esc(x.stat)}</b><span>${esc(x.effect)}</span></div>`).join('')}${model?.note?`<div class="amber">${esc(model.note)}</div>`:''}</div>`;
  }
