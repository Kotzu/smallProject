(()=>{
  ...
  if(selector){
    selector.innerHTML='';
    Object.entries(policies).forEach(([key,p])=>{const b=document.createElement('button');b.dataset.policy=key;b.textContent=p.label+(p.status==='LOCKED'?' · locked':'');selector.appendChild(b);});
  }
