...
local function chooseHemo(ctx)
    ...
    if me.rooted then
        ...
        if canPrep(ctx) then return { action = "Preparation", reason = "Vanish unavailable; Preparation resets Rogue-family cooldowns for a reset" } end
        return { action = "WAIT", reason = "rooted; no verified reset available" }
    end
    if me.stealthed then
        if ctx.range <= 5 and me.energy >= cheapShotEnergy then return { action = "Cheap Shot", reason = "talent-modified opener cost: " .. tostring(cheapShotEnergy) .. " Energy" } end
    ...
    if enemy.casting and me.energy >= 25 and ctx:ready("Kick") then return { action = "Kick", reason = "interrupt dangerous cast" } end
    if me.comboPoints >= 5 and not enemy.stunned and me.energy >= 25 and ctx:ready("Kidney Shot") then return { action = "Kidney Shot", reason = "create full control window" } end
    if me.comboPoints >= 5 and me.energy >= 35 then
        if ctx:ready("Cold Blood") and not me.coldBloodActive and hasTalent(ctx, "Cold Blood") then return { action = "Cold Blood", reason = "talent guarantees finisher crit" } end
        return { action = "Eviscerate", reason = "5 CP finisher" }
    end
    if me.energy >= 35 and hasTalent(ctx, "Hemorrhage") then return { action = "Hemorrhage", reason = "talented builder and pressure" } end
...
local function chooseImprovedSprintBackstab(ctx)
    ...
    if me.stealthed then
        ...
        if not behind and (hasTalent(ctx, "Improved Ambush") or hasTalent(ctx, "Improved Backstab")) then
            return { action = "MOVE_BEHIND", range = 2, reason = "dagger opener requires the verified rear arc" }
        end
        ...
        if me.energy >= cheapShotEnergy then return { action = "Cheap Shot", reason = "build-specific Dirty Deeds cost: " .. tostring(cheapShotEnergy) .. " Energy" } end
        return { action = "WAIT", reason = "pool for opener" }
    end

    if enemy.iceBlock then return { action = "WAIT", reason = "target immune" } end
    if enemy.casting and me.energy >= 25 and ctx:ready("Kick") then return { action = "Kick", reason = "interrupt before spending positional burst" } end
    ...
    -- A stun/incapacitate is a positioning window. Do not waste it hitting from the front
    -- when the build is explicitly centered on Backstab/Ambush.
    if not behind and (enemy.stunned or enemy.incapacitated) and me.energy >= 60 then
        return { action = "MOVE_BEHIND", range = 2, reason = "convert control window into legal Backstab position" }
    end
    ...
    if behind and me.energy >= 60 and hasTalent(ctx, "Improved Backstab") then return { action = "Backstab", reason = "primary dagger builder: Improved Backstab + Opportunity" } end
    if not behind and me.energy >= 45 and ctx:ready("Gouge") then return { action = "Gouge", reason = "Improved Gouge creates a reposition and energy-pool window" } end
    if me.energy >= sinisterStrikeEnergy then return { action = "Sinister Strike", reason = "front-facing fallback at build-modified Energy cost" } end
