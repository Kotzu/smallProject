(()=>{
  ...
  function buildTalentNames(build){
    const out={};
    for(const t of build?.combatTalents||[]){const rank=Number(String(t.rank||'0').split('/')[0])||0;if(rank>0)out[t.name]=rank;}
    return out;
  }

  function sanitizeContext(ctx,build){
    return {self:ctx?.self||{},enemy:ctx?.enemy||{},range:Number(ctx?.range)||0,spec:ctx?.spec||build?.spec||'',buildId:ctx?.buildId||build?.id||'',behindTarget:ctx?.behindTarget===true,readyMap:ctx?.readyMap||{},talentNames:ctx?.talentNames||buildTalentNames(build),build:{id:build?.id||'',modifiers:build?.modifiers||{}}};
  }
...
`;
  ...
  async function choose(className,ctx,build){
    ...
    const raw=window.fengari.load(chunkFor(source,ctx,build),`@${className}/ClassCombat.lua`)();
  ...
  async function selfTest(){
    ...
    const hemo=T?.get?.('rogue_cb_hemo_21_3_27'),dagger=T?.get?.('rogue_imp_sprint_backstab_16_12_23'),mage=T?.get?.('mage_deep_frost_17_0_34');
