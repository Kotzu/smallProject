window.WOW_DUEL=(function(){
  ...
  function makeState(seed,cfg){
    const rng=new E.RNG(seed);
    const rp=D.profiles.rogue_subtlety_lvl60_pvp_bis_p6_baseline,rb=CE.rogue60Baseline(CD.level60.undeadRogue,rp),mp=D.pvpProfiles.mage_frost_gnome_p6_core;
    ...
    const rogueCrit=19+(rb.stats.agi/29)+(rm.meleeCritPct||0);
    ...
    return {
      rng,time:0,range:5,timeline:[],winner:null,cfg,
      rogue:{
        name:'Undead Rogue · Subtlety/Hemo',hp:rb.health,maxHp:rb.health,energy:100,maxEnergy:100,nextEnergy:2000,
        ap:rb.attackPower,armor:rb.armor,hit:rb.hitPct,crit:rogueCrit,dodge:rogueDodge,talents:rm,
        spellRangedHitPenaltyPct:rm.attackerSpellRangedHitPenaltyPct||0,
        ...
        hemoCharges:0,hemoUntil:0,casting:null,hojReady:0,crusaderUntil:0,crusaderProcs:0,bonescytheHeals:0,
        initiativeProcs:0,ruthlessnessProcs:0,relentlessProcs:0,preparationUses:0
      },
      mage:{
        name:'Gnome Mage · Frost',hp:mp.stats.health,maxHp:mp.stats.health,mana:mp.stats.mana,maxMana:mp.stats.mana,
        armor:mageArmor,dodge:mp.stats.dodgePct+(mp.utility?.arenaGrandMaster?.dodgePct||1),spellDamage:mp.stats.spellDamage,spellCrit:mp.stats.spellCritPct,talents:mm,
        frostHit:mp.stats.spellHitPct+(mm.frostFireHitPct||6),fireHit:mp.stats.spellHitPct+(mm.frostFireHitPct||6),
        gcd:0,cd:{},stun:0,root:0,slow:0,slowPct:0,casting:null,schoolLock:{Frost:0,Fire:0,Arcane:0},iceBlock:0,
        iceBarrier:barrier.absorbBase+mp.stats.spellDamage*barrier.spCoeff,
        manaShield:manaShield.absorbBase+(mp.gearEffects?.manaShieldBonusAbsorb||0),agmShield:0,mindNumbingUntil:0,elementalVulnerabilityUntil:0,
  ...
  function log(s,actor,text,kind='event'){s.timeline.push({t:round(s.time/1000,1),actor,text,kind});if(s.timeline.length>700)s.timeline.shift();}
  ...
  function tryFinisherTalents(s,cp){
    const r=s.rogue,t=r.talents||{};
    const relentlessPct=(t.relentlessChancePerComboPct||20)*cp;
    if(relentlessPct>0&&rollPct(s,relentlessPct)){
      r.energy=Math.min(r.maxEnergy,r.energy+(t.relentlessEnergy||25));r.relentlessProcs++;
      log(s,'Rogue',`Relentless Strikes → +${t.relentlessEnergy||25} Energy`,'proc');
    }
    if((t.ruthlessnessProcPct||0)>0&&rollPct(s,t.ruthlessnessProcPct)){
      r.combo=Math.min(5,r.combo+1);r.ruthlessnessProcs++;
      log(s,'Rogue','Ruthlessness → +1 combo point','proc');
    }
  }
  ...
  function physicalSpecial(s,name,base,critMult=2,forceCrit=false,opts={}){
    const m=s.mage,r=s.rogue;if(m.iceBlock>s.time){log(s,'Rogue',`${name}: immune (Ice Block)`,'miss');return {hit:false};}
    const miss=E.sameLevelMeleeSpecialMiss(r.hit);if(rollPct(s,miss)){log(s,'Rogue',`${name}: MISS`,'miss');return {hit:false};}
    const stunned=m.stun>s.time;if(!stunned&&rollPct(s,m.dodge)){log(s,'Rogue',`${name}: DODGE`,'miss');return {hit:false};}
    let dmg=base*(1+(r.talents.murderDamagePct||2)/100);dmg*=1-E.physicalArmorReduction(m.armor,60);dmg=consumeHemoBonus(s,name,dmg);
    const crit=forceCrit||rollPct(s,r.crit);if(crit)dmg*=critMult;
    const dealt=damageMage(s,dmg,name,crit);if(opts.weaponHand&&dealt.dealt+dealt.absorbed>0)procWeaponEffects(s,opts.weaponHand,opts.allowHoJ!==false);return {hit:true,crit,dmg};
  }
  ...
  function spellHit(s,school){
    const hit=school==='Frost'?s.mage.frostHit:s.mage.fireHit;
    const effectiveHit=hit-(s.rogue.spellRangedHitPenaltyPct||0);
    return !rollPct(s,E.sameLevelSpellMiss(effectiveHit));
  }
  function spellDamage(s,key,{frozenBonus=true}={}){
    const sp=D.pvpSpellbooks.Mage[key],r=s.rogue,m=s.mage;if(!spellHit(s,sp.school)){log(s,'Mage',`${sp.name}: RESIST/MISS`,'miss');return {hit:false};}
    let bonusSP=0;if(m.elementalVulnerabilityUntil>s.time){bonusSP=M.frostfire6p.bonusSpellPowerForNextHit;m.elementalVulnerabilityUntil=0;log(s,'Mage',`Elemental Vulnerability consumed → +${bonusSP} effective spell power`,'proc');}
    let dmg=rand(s,sp.damage[0],sp.damage[1])+(m.spellDamage+bonusSP)*sp.spCoeff;
    if(sp.school==='Frost')dmg*=1+(m.talents.frostDamagePct||6)/100;
    if(key==='coneOfCold')dmg*=1+(m.talents.coneOfColdDamagePct||35)/100;
    let critChance=m.spellCrit;if(sp.school==='Frost'&&frozenBonus&&r.root>s.time)critChance+=(m.talents.frozenCritBonusPct||50);
    const crit=rollPct(s,critChance);if(crit)dmg*=sp.school==='Frost'?(m.talents.frostCritMultiplier||2):1.5;
    damageRogue(s,dmg,sp.name,crit);
    if(key==='frostbolt'){r.slow=s.time+(9000+(m.talents.chillDurationBonusMs||3000));r.slowPct=40+(m.talents.chillSlowBonusPct||10);}
    if(key==='coneOfCold'){r.slow=s.time+(8000+(m.talents.chillDurationBonusMs||3000));r.slowPct=50+(m.talents.chillSlowBonusPct||10);}
    if(rollPct(s,M.frostfire6p.procPct)){m.elementalVulnerabilityUntil=s.time+M.frostfire6p.debuffMs;log(s,'Mage','Frostfire 6p → Elemental Vulnerability primed for next damaging spell','proc');}
    return {hit:true,crit,dmg};
  }

  function rogueCheapShot(s){
    const r=s.rogue,sp=D.pvpSpellbooks.Rogue.cheapShot,cost=r.talents.cheapShotEnergy||40;if(!sp||!gcdReady(s,r)||!r.stealth||s.range>5)return false;if(!spend(r,'energy',cost))return false;
    r.gcd=s.time+1000;r.stealth=false;let cp=2;
    if((r.talents.initiativeProcPct||0)>0&&rollPct(s,r.talents.initiativeProcPct)){cp++;r.initiativeProcs++;log(s,'Rogue','Initiative → Cheap Shot gains +1 combo point','proc');}
    r.combo=Math.min(5,r.combo+cp);s.mage.stun=s.time+4000;log(s,'Rogue',`Cheap Shot → 4.0s stun, ${cp} CP (${cost} Energy)`,'control');return true;
  }
  ...
  function rogueKidney(s){
    const r=s.rogue,m=s.mage;if(!gcdReady(s,r)||s.range>5||r.combo<2||r.energy<25||!ready(s,r,'kidney'))return false;
    const cp=r.combo;r.energy-=25;r.combo=0;r.gcd=s.time+1000;setCd(s,r,'kidney',20000);tryFinisherTalents(s,cp);
    if(rollPct(s,E.sameLevelMeleeSpecialMiss(r.hit))){log(s,'Rogue','Kidney Shot: MISS','miss');return true;}
    if(m.stun<=s.time&&rollPct(s,m.dodge)){log(s,'Rogue','Kidney Shot: DODGE','miss');return true;}
    m.stun=s.time+(cp+1)*1000;log(s,'Rogue',`Kidney Shot ${cp} CP → ${cp+1}.0s stun (separate DR from Cheap Shot)`,'control');return true;
  }
  function rogueEvis(s){
    const r=s.rogue;if(!gcdReady(s,r)||s.range>5||r.combo<1||r.energy<35)return false;
    const cp=r.combo,sp=D.pvpSpellbooks.Rogue.eviscerate,range=sp.baseByCombo[cp];r.energy-=35;r.combo=0;r.gcd=s.time+1000;
    const base=(rand(s,range[0],range[1])+currentRogueAP(s)*0.03*cp)*(1+(r.talents.eviscerateDamagePct||15)/100);
    const force=r.coldBlood;r.coldBlood=false;physicalSpecial(s,'Eviscerate',base,2,force);tryFinisherTalents(s,cp);return true;
  }
  ...
  function roguePreparation(s){
    const r=s.rogue,sp=D.pvpSpellbooks.Rogue.preparation;if(!r.talents.preparation||!sp||!ready(s,r,'preparation')||!gcdReady(s,r))return false;
    setCd(s,r,'preparation',sp.cooldownMs||600000);r.gcd=s.time+(sp.gcdMs||1000);
    ['vanish','sprint','kick','kidney','coldBlood'].forEach(k=>r.cd[k]=s.time);
    r.preparationUses++;log(s,'Rogue','Preparation → Rogue-family cooldowns reset (Preparation excluded)','control');return true;
  }
  ...
  function chooseRogue(s){
    ...
    if(r.root>s.time){if(ready(s,r,'vanish')){rogueVanish(s);return;}if(ready(s,r,'preparation')&&r.talents.preparation){roguePreparation(s);return;}return;}
    ...
    if(r.combo>=5&&r.energy>=35){if(ready(s,r,'coldBlood')&&r.talents.coldBlood)rogueColdBlood(s);rogueEvis(s);return;}
    rogueHemo(s);
  }
  ...
  function finish(s){
    ...
    return {seed:s.rng.state,winner:s.winner,duration:round(s.time/1000,1),timeline:s.timeline,
      final:{rogue:{hp:Math.round(s.rogue.hp),energy:Math.round(s.rogue.energy),combo:s.rogue.combo,crusaderProcs:s.rogue.crusaderProcs,bonescytheHeals:s.rogue.bonescytheHeals,initiativeProcs:s.rogue.initiativeProcs,ruthlessnessProcs:s.rogue.ruthlessnessProcs,relentlessProcs:s.rogue.relentlessProcs,preparationUses:s.rogue.preparationUses},mage:{hp:Math.round(s.mage.hp),mana:Math.round(s.mage.mana)},range:round(s.range,1)},
      talentRuntime:{initiativePct:s.rogue.talents.initiativeProcPct||0,ruthlessnessPct:s.rogue.talents.ruthlessnessProcPct||0,preparation:!!s.rogue.talents.preparation,elusivenessMs:s.rogue.talents.elusivenessCooldownReductionMs||0,heightenedSensesHitPenaltyPct:s.rogue.spellRangedHitPenaltyPct},
      calibrationRequired:[]};
  }
  function run(seed,cfg){
    const gate=canRun(cfg);if(!gate.ready)return {error:'STRICT_DATA_GATE',missing:gate.missing};const s=makeState(seed,cfg);
    log(s,'System',`v0.18 talent-aware duel starts at 5 yd. Rogue stealthed; Mage pre-buffed. Initiative, Ruthlessness, Preparation, Elusiveness and registered build modifiers are active.`,'start');
    for(;s.time<=MAX_TIME&&alive(s);s.time+=STEP){resources(s);completeMageCast(s);if(!alive(s))break;chooseRogue(s);if(!alive(s))break;processAutos(s);if(!alive(s))break;chooseMage(s);if(!alive(s))break;move(s);}return finish(s);
  }
  function batch(seed,count,cfg){const gate=canRun(cfg);if(!gate.ready)return {error:'STRICT_DATA_GATE',missing:gate.missing};const n=clamp(Number(count)||1000,1,100000),wins={Rogue:0,Mage:0,Draw:0,Timeout:0};let duration=0;const base=(Number(seed)||1)>>>0;for(let i=0;i<n;i++){const r=run((base+Math.imul(i+1,2654435761))>>>0,cfg);wins[r.winner]=(wins[r.winner]||0)+1;duration+=r.duration;}return {count:n,wins,rates:Object.fromEntries(Object.entries(wins).map(([k,v])=>[k,round(v*100/n,2)])),avgDuration:round(duration/n,2)};}
  function selfTest(cfg){const a=run(1337,cfg),b=run(1337,cfg);return {deterministic:JSON.stringify(a)===JSON.stringify(b),winner:a.winner,duration:a.duration,version:'0.18.0'};}
  return {version:'0.18.0',canRun,run,batch,selfTest,constants:{baseRunYdPerSec:BASE_RUN,stepMs:STEP,maxTimeMs:MAX_TIME}};
})();
