(()=>{
  ...
  const KEY='wow-pvp-sim.rogue-learning.v025';
  ...
  const fresh=()=>({version:'0.25',generation:3,observedFights:0,rogueWins:0,rogueLosses:0,champion:copy(initial()),candidateIndex:0,candidate:null,evidence:null,history:[]});
  let state=load()||fresh();
  if(state.version!=='0.25')state=fresh();
  state.champion=duel.normalizeRoguePolicy(state.champion||initial());

  // Only decision-policy fields are mutated. Gear, talents and WoW formulas are immutable.
  const variants=[
    ['kidneyMinCp',4,'try Kidney Shot at 4+ CP'],
    ['kidneyEnergyReserve',20,'reserve 20 Energy for the follow-up after Kidney Shot'],
    ...
    ['hemoMinEnergy',65,'pool to 65 Energy before Hemorrhage']
  ];

  function candidateFor(index){
    const [field,value,reason]=variants[index%variants.length];
    return {policy:duel.normalizeRoguePolicy({...state.champion,[field]:value,id:`rogue_g${state.generation}_c${index}_${field}`,version:(state.champion.version||3)+1}),field,value,reason};
  }
  function ensureCandidate(){
    if(!state.candidate){
      state.candidate=candidateFor(state.candidateIndex);
  ...
  function trainChunk(cfg,seed,count=250){
    const c=ensureCandidate(),cmp=duel.comparePolicies(seed,count,cfg,state.champion,c.policy);
    if(cmp.error)return {error:cmp.error};
    addEvidence(cmp);const promotion=maybePromote();save(state);
    return {candidate:copy(c),comparison:cmp,evidence:copy(state.evidence),promotion};
  }
  function observe(result,cfg){
    if(!result||result.error)return null;
    state.observedFights++;if(result.winner==='Rogue')state.rogueWins++;else if(result.winner==='Mage')state.rogueLosses++;
    const diagnosis=diagnose(result),seed=((Number(result.seed)||1)+(state.observedFights*104729))>>>0,training=trainChunk(cfg,seed,250);
    const summary={observedFights:state.observedFights,rogueWins:state.rogueWins,rogueLosses:state.rogueLosses,generation:state.generation,champion:copy(state.champion),diagnosis,training};save(state);return summary;
  }
  ...
  window.WOW_ROGUE_LEARNER={version:'0.25',getChampionPolicy,getState,observe,trainChunk,reset,variants:copy(variants)};
  window.WOW_DUEL={...duel,run,version:'0.25-learning',learningVersion:'0.25'};
})();
