window.WOW_DUEL=(function(){
  ...
  const DEFAULT_ROGUE_POLICY=Object.freeze({
    id:'rogue_cb_hemo_baseline_v017',
    version:1,
    vanishOnRoot:true,
    kickEnabled:true,
  ...
  function runtimePolicy(cfg){
    const learned=window.WOW_ROGUE_LEARNER?.getChampionPolicy?.();
    return normalizePolicy(cfg?.roguePolicy||learned||D.roguePolicyChampion||DEFAULT_ROGUE_POLICY);
  }
  ...
  function selectedBuild(cfgSide,fallbackId){
    const id=cfgSide?.build||fallbackId;
    return D.talentBuildLibrary?.[id]||D.pvpBuilds?.[id]||D.pvpBuilds?.[fallbackId]||{modifiers:{}};
  }
  ...
  function makeState(seed,cfg,policy){
    ...
    const rbld=selectedBuild(cfg?.a,'rogue_cb_hemo_21_3_27'),mbld=selectedBuild(cfg?.b,'mage_deep_frost_17_0_34');
    const rm=rbld.modifiers||{},mm=mbld.modifiers||{};
    ...
    const mageArmor=Math.trunc(mp.stats.armorBeforeTalents+mp.stats.intellect*((mm.armorFromIntellectPct||0)/100));
    const barrier=D.pvpSpellbooks.Mage.iceBarrier,manaShield=D.pvpSpellbooks.Mage.manaShield;
    return {rng,time:0,range:5,timeline:[],winner:null,policy,config:cfg,
      rogue:{name:'Undead Rogue · Subtlety/Hemo',hp:rb.health,maxHp:rb.health,energy:100,maxEnergy:100,nextEnergy:2000,ap:rb.attackPower,armor:rb.armor,hit:rb.hitPct,crit:rogueCrit,dodge:rogueDodge,talent:rm,
        ...
        combo:0,gcd:0,cd:{},stun:0,root:0,slow:0,slowPct:0,stealth:true,sprint:0,coldBlood:false,hemoCharges:0,hemoUntil:0,casting:null,hojReady:0,crusaderUntil:0,crusaderProcs:0,bonescytheHeals:0,decisions:{}},
      mage:{name:'Gnome Mage · Frost',hp:mp.stats.health,maxHp:mp.stats.health,mana:mp.stats.mana,maxMana:mp.stats.mana,armor:mageArmor,dodge:mp.stats.dodgePct+(mp.utility?.arenaGrandMaster?.dodgePct||1),spellDamage:mp.stats.spellDamage,spellCrit:mp.stats.spellCritPct,talent:mm,
        frostHit:mp.stats.spellHitPct+(mm.frostFireHitPct||0),fireHit:mp.stats.spellHitPct+(mm.frostFireHitPct||0),gcd:0,cd:{},stun:0,root:0,slow:0,slowPct:0,casting:null,schoolLock:{Frost:0,Fire:0,Arcane:0},iceBlock:0,
        iceBarrier:barrier.absorbBase+mp.stats.spellDamage*barrier.spCoeff,manaShield:manaShield.absorbBase+(mp.gearEffects?.manaShieldBonusAbsorb||0),agmShield:0,mindNumbingUntil:0,elementalVulnerabilityUntil:0,
        mageArmor:true,resist:{...mp.stats.resist,arcane:(mp.stats.resist.arcane||0)+15,fire:(mp.stats.resist.fire||0)+15,frost:(mp.stats.resist.frost||0)+15,nature:(mp.stats.resist.nature||0)+15,shadow:(mp.stats.resist.shadow||0)+15}}
    };
  }
  ...
  function mageCost(base,school){return (school==='Frost'||school==='Fire')?Math.trunc(base*(1-(M.elementalPrecision?.manaCostReductionPct||0)/100)):base;}
  ...
  function physicalSpecial(s,name,base,critMult=2,forceCrit=false,opts={}){const m=s.mage;if(m.iceBlock>s.time){log(s,'Rogue',`${name}: immune (Ice Block)`,'miss');return {hit:false};}const miss=E.sameLevelMeleeSpecialMiss(s.rogue.hit);if(rollPct(s,miss)){log(s,'Rogue',`${name}: MISS`,'miss');return {hit:false};}const stunned=m.stun>s.time;if(!stunned&&rollPct(s,m.dodge)){log(s,'Rogue',`${name}: DODGE`,'miss');return {hit:false};}let dmg=base*(1+(s.rogue.talent.murderDamagePct||0)/100);dmg*=1-E.physicalArmorReduction(m.armor,60);dmg=consumeHemoBonus(s,name,dmg);const crit=forceCrit||rollPct(s,s.rogue.crit);if(crit)dmg*=critMult;const dealt=damageMage(s,dmg,name,crit);if(opts.weaponHand&&dealt.dealt+dealt.absorbed>0)procWeaponEffects(s,opts.weaponHand,opts.allowHoJ!==false);return {hit:true,crit,dmg};}
  ...
  function spellDamage(s,key,{frozenBonus=true}={}){const sp=D.pvpSpellbooks.Mage[key],r=s.rogue;if(!spellHit(s,sp.school)){log(s,'Mage',`${sp.name}: RESIST/MISS`,'miss');return {hit:false};}let bonusSP=0;if(s.mage.elementalVulnerabilityUntil>s.time){bonusSP=M.frostfire6p.bonusSpellPowerForNextHit;s.mage.elementalVulnerabilityUntil=0;log(s,'Mage',`Elemental Vulnerability consumed → +${bonusSP} effective spell power`,'proc');}let dmg=rand(s,sp.damage[0],sp.damage[1])+(s.mage.spellDamage+bonusSP)*sp.spCoeff;if(sp.school==='Frost')dmg*=1+(s.mage.talent.frostDamagePct||0)/100;if(key==='coneOfCold')dmg*=1+(s.mage.talent.coneOfColdDamagePct||0)/100;let critChance=s.mage.spellCrit;if(sp.school==='Frost'&&frozenBonus&&r.root>s.time)critChance+=(s.mage.talent.frozenCritBonusPct||0);const crit=rollPct(s,critChance);if(crit)dmg*=sp.school==='Frost'?(s.mage.talent.frostCritMultiplier||1.5):1.5;damageRogue(s,dmg,sp.name,crit);if(key==='frostbolt'){r.slow=s.time+sp.slowMs+(M.permafrost?.slowDurationBonusMs||0);r.slowPct=sp.slowPct+(M.permafrost?.slowPctBonus||0);}if(key==='coneOfCold'){r.slow=s.time+sp.slowMs+(M.permafrost?.slowDurationBonusMs||0);r.slowPct=sp.slowPct+(M.permafrost?.slowPctBonus||0);}if(rollPct(s,M.frostfire6p.procPct)){s.mage.elementalVulnerabilityUntil=s.time+M.frostfire6p.debuffMs;log(s,'Mage','Frostfire 6p → Elemental Vulnerability primed for next damaging spell','proc');}return {hit:true,crit,dmg};}

  function relentless(s,cp){const chance=(s.rogue.talent.relentlessChancePerComboPct||20)*cp;if(rollPct(s,chance))s.rogue.energy=Math.min(100,s.rogue.energy+(s.rogue.talent.relentlessEnergy||25));}
  function rogueCheapShot(s){const sp=D.pvpSpellbooks.Rogue.cheapShot,cost=s.rogue.talent.cheapShotEnergy||sp.cost;if(!sp||!gcdReady(s,s.rogue)||!s.rogue.stealth||s.range>5)return false;if(!spend(s.rogue,'energy',cost))return false;s.rogue.gcd=s.time+1000;s.rogue.stealth=false;s.rogue.combo=Math.min(5,s.rogue.combo+2);s.mage.stun=s.time+sp.stunMs;log(s,'Rogue',`Cheap Shot → ${(sp.stunMs/1000).toFixed(1)}s stun, 2 CP`,'control');return true;}
  function rogueHemo(s){if(!gcdReady(s,s.rogue)||s.range>5||s.rogue.energy<35)return false;s.rogue.energy-=35;s.rogue.gcd=s.time+1000;const base=weaponRoll(s,'mh');const hit=physicalSpecial(s,'Hemorrhage',base,s.rogue.talent.hemorrhageCritMultiplier||2,false,{weaponHand:'mh'});if(hit.hit){s.rogue.combo=Math.min(5,s.rogue.combo+1);s.rogue.hemoCharges=30;s.rogue.hemoUntil=s.time+15000;if(hit.crit)s.rogue.energy=Math.min(100,s.rogue.energy+M.bonescythe4p.energyOnBuilderCrit);}return true;}
  function rogueKidney(s){if(!gcdReady(s,s.rogue)||s.range>5||s.rogue.combo<2||s.rogue.energy<25||!ready(s,s.rogue,'kidney'))return false;const cp=s.rogue.combo;s.rogue.energy-=25;s.rogue.combo=0;s.rogue.gcd=s.time+1000;setCd(s,s.rogue,'kidney',20000);if(rollPct(s,E.sameLevelMeleeSpecialMiss(s.rogue.hit))){log(s,'Rogue','Kidney Shot: MISS','miss');return true;}if(s.mage.stun<=s.time&&rollPct(s,s.mage.dodge)){log(s,'Rogue','Kidney Shot: DODGE','miss');return true;}s.mage.stun=s.time+(cp+1)*1000;relentless(s,cp);log(s,'Rogue',`Kidney Shot ${cp} CP → ${cp+1}.0s stun`,'control');return true;}
  function rogueEvis(s){if(!gcdReady(s,s.rogue)||s.range>5||s.rogue.combo<1||s.rogue.energy<35)return false;const cp=s.rogue.combo,sp=D.pvpSpellbooks.Rogue.eviscerate,range=sp.baseByCombo[cp];s.rogue.energy-=35;s.rogue.combo=0;s.rogue.gcd=s.time+1000;const base=(rand(s,range[0],range[1])+currentRogueAP(s)*sp.apCoeffPerCombo*cp)*(1+(s.rogue.talent.eviscerateDamagePct||0)/100);const force=s.rogue.coldBlood;s.rogue.coldBlood=false;physicalSpecial(s,'Eviscerate',base,2,force);relentless(s,cp);return true;}
  function rogueColdBlood(s){if(!ready(s,s.rogue,'coldBlood')||s.rogue.coldBlood||!s.rogue.talent.coldBlood)return false;s.rogue.coldBlood=true;setCd(s,s.rogue,'coldBlood',180000);log(s,'Rogue','Cold Blood → next eligible attack guaranteed crit','buff');return true;}
  ...
  function rogueVanish(s){const r=s.rogue;if(!ready(s,r,'vanish'))return false;setCd(s,r,'vanish',300000);r.root=0;r.slow=0;r.slowPct=0;r.stealth=true;log(s,'Rogue','Vanish → root/slow removed, Stealth','control');return true;}
  ...
  function mageNova(s){const m=s.mage,sp=D.pvpSpellbooks.Mage.frostNova,cost=mageCost(sp.cost,'Frost');if(!gcdReady(s,m)||!ready(s,m,'nova')||m.schoolLock.Frost>s.time||m.mana<cost||s.range>10)return false;m.mana-=cost;m.gcd=s.time+1500;setCd(s,m,'nova',m.talent.frostNovaCooldownMs||25000);const result=spellDamage(s,'frostNova',{frozenBonus:false});if(result.hit){s.rogue.root=s.time+sp.rootMs;log(s,'Mage',`Frost Nova → Rogue rooted ${(sp.rootMs/1000).toFixed(1)}s`,'control');}return true;}
  ...
  function startFrostbolt(s){const m=s.mage,sp=D.pvpSpellbooks.Mage.frostbolt,cost=mageCost(sp.cost,'Frost');if(!gcdReady(s,m)||m.casting||m.schoolLock.Frost>s.time||m.mana<cost||s.range>30)return false;m.mana-=cost;m.gcd=s.time+1500;const castMs=mageCastTime(s,m.talent.frostboltCastMs||sp.castMs);m.casting={key:'frostbolt',school:'Frost',end:s.time+castMs,range:30};log(s,'Mage',`Frostbolt cast started (${(castMs/1000).toFixed(1)}s${castMs>(m.talent.frostboltCastMs||sp.castMs)?' · Mind-numbing':''})`,'cast');return true;}
  ...
  function mageBarrier(s){const m=s.mage,sp=D.pvpSpellbooks.Mage.iceBarrier,cost=mageCost(sp.cost,'Frost');if(!gcdReady(s,m)||!ready(s,m,'barrier')||m.schoolLock.Frost>s.time||m.mana<cost||!m.talent.iceBarrier)return false;m.mana-=cost;m.gcd=s.time+1500;setCd(s,m,'barrier',sp.cooldownMs);m.iceBarrier=sp.absorbBase+m.spellDamage*sp.spCoeff;log(s,'Mage',`Ice Barrier R4 → ${Math.round(m.iceBarrier)} absorb`,'buff');return true;}
  ...
  function mageIceBlock(s){const m=s.mage;if(!gcdReady(s,m)||!ready(s,m,'iceBlock')||m.schoolLock.Frost>s.time||m.mana<15||!m.talent.iceBlock)return false;m.mana-=15;m.gcd=s.time+1500;setCd(s,m,'iceBlock',300000);m.iceBlock=s.time+2500;m.stun=0;m.root=0;log(s,'Mage','Ice Block → immune; policy cancels after 2.5s (max duration 10s)','control');return true;}
  function mageColdSnap(s){const m=s.mage;if(!ready(s,m,'coldSnap')||!m.talent.coldSnap)return false;setCd(s,m,'coldSnap',600000);['nova','cone','barrier','iceBlock'].forEach(k=>m.cd[k]=s.time);log(s,'Mage','Cold Snap → Frost cooldowns reset','control');return true;}

  function chooseRogue(s){
    const r=s.rogue,m=s.mage,p=s.policy;
    if(r.stun>s.time)return;
    if(r.root>s.time){if(p.vanishOnRoot&&ready(s,r,'vanish')){decision(s,'Vanish','root break / reopen');rogueVanish(s);}return;}
    if(r.stealth){if(s.range<=5&&r.energy>=(r.talent.cheapShotEnergy||40)){decision(s,'Cheap Shot','stealth opener');rogueCheapShot(s);}return;}
    ...
    if(m.casting&&p.kickEnabled){const remaining=m.casting.end-s.time;if(remaining>=p.kickMinRemainingMs&&rogueKick(s)){decision(s,'Kick',`interrupt with ${remaining}ms cast remaining`);return;}}
    if(r.combo>=p.kidneyMinCp&&m.stun<=s.time&&r.energy>=25+p.kidneyEnergyReserve&&ready(s,r,'kidney')){decision(s,'Kidney Shot',`${r.combo} CP control window`);rogueKidney(s);return;}
    const execute=m.hp/m.maxHp*100<=p.executeEvisHpPct&&r.combo>=p.executeEvisMinCp;
    if((r.combo>=p.evisMinCp||execute)&&r.energy>=35){if(r.combo>=p.coldBloodMinCp&&ready(s,r,'coldBlood')&&r.talent.coldBlood){decision(s,'Cold Blood',`${r.combo} CP finisher setup`);rogueColdBlood(s);}decision(s,'Eviscerate',`${r.combo} CP${execute?' execute':''}`);rogueEvis(s);return;}
  ...
  function finish(s){if(s.rogue.hp<=0&&s.mage.hp<=0)s.winner='Draw';else if(s.mage.hp<=0)s.winner='Rogue';else if(s.rogue.hp<=0)s.winner='Mage';else s.winner='Timeout';log(s,'System',`WINNER: ${s.winner}`,'winner');return {seed:s.rng.state,winner:s.winner,duration:round(s.time/1000,1),timeline:s.timeline,policy:{...s.policy},decisions:{...s.rogue.decisions},final:{rogue:{hp:Math.round(s.rogue.hp),maxHp:s.rogue.maxHp,energy:Math.round(s.rogue.energy),combo:s.rogue.combo,crusaderProcs:s.rogue.crusaderProcs,bonescytheHeals:s.rogue.bonescytheHeals},mage:{hp:Math.round(s.mage.hp),maxHp:s.mage.maxHp,mana:Math.round(s.mage.mana),maxMana:s.mage.maxMana},range:round(s.range,1)},calibrationRequired:[]};}
  function runWithPolicy(seed,cfg,policy){const gate=canRun(cfg);if(!gate.ready)return {error:'STRICT_DATA_GATE',missing:gate.missing};const p=normalizePolicy(policy),s=makeState(seed,cfg,p);log(s,'System',`v0.17 precise duel · Rogue policy ${p.id} · talents applied from selected builds.`,'start');for(;s.time<=MAX_TIME&&alive(s);s.time+=STEP){resources(s);completeMageCast(s);if(!alive(s))break;chooseRogue(s);if(!alive(s))break;processAutos(s);if(!alive(s))break;chooseMage(s);if(!alive(s))break;move(s);}return finish(s);}
  function run(seed,cfg){return runWithPolicy(seed,cfg,runtimePolicy(cfg));}
  function batchWithPolicy(seed,count,cfg,policy){
    const gate=canRun(cfg);if(!gate.ready)return {error:'STRICT_DATA_GATE',missing:gate.missing};
    const n=clamp(Number(count)||1000,1,100000),wins={Rogue:0,Mage:0,Draw:0,Timeout:0};let duration=0,rogueHp=0,mageHp=0;
    const base=(Number(seed)||1)>>>0,p=normalizePolicy(policy);
    for(let i=0;i<n;i++){const r=runWithPolicy((base+Math.imul(i+1,2654435761))>>>0,cfg,p);wins[r.winner]=(wins[r.winner]||0)+1;duration+=r.duration;rogueHp+=r.final.rogue.hp/r.final.rogue.maxHp;mageHp+=r.final.mage.hp/r.final.mage.maxHp;}
    const rates=Object.fromEntries(Object.entries(wins).map(([k,v])=>[k,round(v*100/n,2)]));
    const avgRogueHpPct=round(rogueHp*100/n,3),avgMageHpPct=round(mageHp*100/n,3);
    const score=round(rates.Rogue*100+(avgRogueHpPct-avgMageHpPct)-rates.Timeout*2,4);
    return {count:n,wins,rates,avgDuration:round(duration/n,3),avgRogueHpPct,avgMageHpPct,score,policy:p};
  }
  ...
  function selfTest(cfg){const p=runtimePolicy(cfg),a=runWithPolicy(1337,cfg,p),b=runWithPolicy(1337,cfg,p);return {deterministic:JSON.stringify(a)===JSON.stringify(b),winner:a.winner,duration:a.duration,version:'0.17.0',policy:p.id};}
  return {version:'0.17.0',canRun,run,runWithPolicy,batch,batchWithPolicy,comparePolicies,selfTest,defaultRoguePolicy:{...DEFAULT_ROGUE_POLICY},normalizeRoguePolicy:normalizePolicy,constants:{baseRunYdPerSec:BASE_RUN,stepMs:STEP,maxTimeMs:MAX_TIME}};
})();
