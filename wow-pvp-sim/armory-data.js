window.WOW_ARMORY=(function(){
  ...
  const rogueEnchantBySlot={Head:'deaths_embrace_head',Shoulder:'might_scourge',Back:'cloak_stealth',Chest:'major_health',Wrist:'superior_stamina',Hands:'superior_agility_gloves',Legs:'deaths_embrace_legs',Feet:'minor_speed','Main Hand':'crusader','Off Hand':'weapon_agility',Ranged:'sniper_scope'};
  const mageItems={
  ...
  function rogueArmory(){
    const D=window.WOW_DATA,p=D?.profiles?.rogue_subtlety_lvl60_pvp_bis_p6_baseline;if(!p)return null;
    return SLOT_ORDER.map(slot=>{const id=p.slots[slot],item=id?D.items?.[String(id)]:null,enchKey=rogueEnchantBySlot[slot],ench=enchKey?D.enchants?.[enchKey]:null;return {slot,id:id||null,name:item?.name||'Empty',verified:!!item?.verified,enchant:ench?.name||null,enchantVerified:enchKey?!!ench?.verified:true,gem:null,socketNote:'Classic Era: no gem socket'};});
  }
  ...
  function audit(s){
    ...
    if(isRogue(s)){
      const p=D?.profiles?.rogue_subtlety_lvl60_pvp_bis_p6_baseline,C=p?.combinedStaticTotals;
      ...
      checks.push({name:'STR total',pass:eq(stats.str,engine.stats.str),value:engine.stats.str},{name:'AGI total',pass:eq(stats.agi,engine.stats.agi),value:engine.stats.agi},{name:'STA total',pass:eq(stats.sta,engine.stats.sta),value:engine.stats.sta},{name:'HP formula',pass:eq(independent.hp,engine.health),value:engine.health},{name:'AP formula',pass:eq(independent.meleeAP,engine.attackPower),value:engine.attackPower},{name:'Armor formula',pass:eq(independent.armor,engine.armor),value:engine.armor});
      const pass=itemIdsOk&&checks.every(x=>x.pass);
      return {pass,status:pass?'PASS':'FAIL',reason:pass?'gear → primary stats → HP/AP/Armor recalculate identic în două căi':'Rogue math mismatch',mapping:'FULL',checks};
    }
    if(isMage(s)){
      ...
      const manaExpected=1213+W.manaFromInt(p.stats.intellect),armorExpected=Math.trunc(p.stats.armorBeforeTalents+p.stats.intellect*.5);
      const intCritContribution=p.stats.intellect*(W.classes.Mage.per.intSpellCrit||0);
  ...
  return {SLOT_ORDER,armoryFor,audit,statModel,isRogue,isMage,version:'0.11-armory-audit-math'};
})();