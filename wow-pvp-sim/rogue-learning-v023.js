(()=>{
  ...
  const KEY='wow-pvp-sim.rogue-learning.v023';
  ...
  const fresh=()=>({version:'0.23',generation:1,observedFights:0,rogueWins:0,rogueLosses:0,champion:copy(initial()),candidateIndex:0,candidate:null,evidence:null,history:[]});
  let state=load()||fresh();
  if(state.version!=='0.23')state=fresh();
  ...
  const variants=[
    ...
    ['executeEvisHpPct',25,'use 4+ CP Eviscerate below 25% target HP'],
  ...
  function candidateFor(index){
    ...
    return {policy:duel.normalizeRoguePolicy({...state.champion,[field]:value,id:`rogue_g${state.generation}_c${index}_${field}`,version:(state.champion.version||2)+1}),field,value,reason};
  }
  ...
  window.WOW_ROGUE_LEARNER={version:'0.23',getChampionPolicy,getState,observe,trainChunk,reset,variants:copy(variants)};
  window.WOW_DUEL={...duel,run,version:'0.23-learning',learningVersion:'0.23'};
})();
