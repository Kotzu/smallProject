(()=>{
  ...
  const library={
    rogue_cb_hemo_21_3_27:{
      ...
      sourceNote:'Classic Cold Blood Hemorrhage PvP build. Active numeric effects are explicitly registered and kernel-calibrated.'
    },
    ...
    rogue_imp_sprint_backstab_17_12_22:{
      ...
      calculatorUrl:'https://www.wowhead.com/classic/talent-calc/rogue/005320124-320302002-05024303030011',
      combatTalents:[
        {name:'Malice',rank:'5/5',effect:'+5% melee critical strike chance',modifier:'meleeCritPct',value:5},
        ...
        {name:'Improved Gouge',rank:'3/3',effect:'+1.5 sec Gouge duration',modifier:'gougeDurationBonusMs',value:1500},
        ...
        {name:'Preparation',rank:'1/1',effect:'reset-oriented PvP policy remains available',modifier:'preparation',value:true},
      ...
      kernelBlockers:[
        'Backstab/Ambush positional damage path must be calibrated in the web kernel',
        'Dagger loadout must pass independent stat audit',
        'Improved Sprint root/snare removal must be driven by ClassCombat policy rather than Hemo policy'
      ],
      sourceNote:'Traditional Improved Sprint Backstab variant published in Blizzard Classic discussion: calculator 005320124-320302002-05024303030011. Individual numeric talent effects are cross-checked against Classic Wowhead talent spell pages.'
    },
  ...
  function defaultBuild(className,spec){
    const ids=idsFor(className,spec);
    return ids.find(id=>library[id]?.status==='ACTIVE_CALIBRATED')||ids[0]||null;
  }
  ...
  function auditSelection(side){
    ...
    return {
      pass:true,
      status:active?'PASS':'VERIFIED_LOCKED',
      buildId:id,
      build:b,
      kernelReady:active,
      reason:active?`${b.name} · ${b.points} · calibrated`:`${b.name} · ${b.points} · verified build, kernel locked`
    };
  }

  function kernelAudit(cfg){
    const a=auditSelection(cfg?.a),b=auditSelection(cfg?.b),missing=[];
    if(!a.pass)missing.push('Player A talent build: '+a.reason);
    else if(!a.kernelReady)missing.push(`Player A build ${a.build.name} is verified but not kernel-calibrated`);
    else if(a.buildId!==kernelBindings.a)missing.push('Player A build not supported by current kernel');
    if(!b.pass)missing.push('Player B talent build: '+b.reason);
    else if(!b.kernelReady)missing.push(`Player B build ${b.build.name} is verified but not kernel-calibrated`);
    else if(b.buildId!==kernelBindings.b)missing.push('Player B build not supported by current kernel');
  ...
  window.WOW_TALENTS={version:'0.17',library,buildIds,idsFor,get,defaultBuild,auditSelection,kernelAudit,kernelBindings,pointTotal};
})();
