(()=>{
  ...
  function playerState(p){return{class:$(p+'c')?.value,spec:$(p+'s')?.value,race:$(p+'r')?.value,gear:$(p+'g')?.value,build:$(p+'build')?.value||null};}
  function refreshSelect(p){
    const el=$(p+'build');if(!el)return;
    const s=playerState(p),ids=T.idsFor(s.class,s.spec),previous=el.value;
    if(!ids.length){el.innerHTML='<option value="">LOCKED — no verified build</option>';el.disabled=true;}
    else{
      el.disabled=false;
      el.innerHTML=ids.map(id=>{const b=T.get(id),suffix=b.status==='ACTIVE_CALIBRATED'?'':' · KERNEL LOCKED';return `<option value="${esc(id)}">${esc(b.name)} · ${esc(b.points)}${suffix}</option>`;}).join('');
  ...
  function renderPlayer(p){
    ...
    root.innerHTML=`
      ...
      <div class="talent-summary"><b>Combat modifiers:</b> ${esc(modifierText(b)||'verified by build registry')}</div>
      ${blockers?`<div class="talent-lock-reasons"><b>Nu intră încă în Fight:</b><ul>${blockers}</ul></div>`:''}
