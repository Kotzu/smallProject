(function(){
  const D=window.WOW_DATA,E=window.WOW_ENGINE,CD=window.WOW_CHARACTER_DATA,CE=window.WOW_CHARACTER_ENGINE,DUEL=window.WOW_DUEL,A=window.WOW_ARMORY;
  ...
  function bind(p){
    ...
    function refreshSpecRace(){state[p].class=c.value;fillSelect(s,specsByClass[state[p].class],state[p].spec);state[p].spec=s.value;fillSelect(r,RACES[state[p].class],state[p].race);state[p].race=r.value;renderStatus(p);}
    c.onchange=refreshSpecRace;s.onchange=()=>{state[p].spec=s.value;renderStatus(p)};r.onchange=()=>{state[p].race=r.value;renderStatus(p)};g.onchange=()=>{state[p].gear=g.value;renderStatus(p)};refreshSpecRace();
  }
  function renderStatus(p){
    const s=state[p],prof=exactProfile(s),audit=A?.audit(s);
    let txt=prof?'<span class="green">✓ kernel profile</span>':'<span class="amber">date exacte încă neîncărcate</span>';
    if(audit)txt+=audit.pass?' · <span class="green">STAT AUDIT PASS</span>':` · <span class="red">STAT AUDIT FAIL</span>`;
  ...
  function updateGate(){
    if(!DUEL)return;
    const cfg=config(),gate=DUEL.canRun(cfg),aa=A?.audit(cfg.a),ab=A?.audit(cfg.b),auditReady=!!aa?.pass&&!!ab?.pass,ready=gate.ready&&auditReady;
    $('runBtn').disabled=!ready;setBatchButtons(!ready);
    if(ready){const t=DUEL.selfTest(cfg);$('gateReason').innerHTML=`<span class="green">RUNNABLE v${DUEL.version} · A/B STAT AUDIT PASS · reproducibil ${t.deterministic?'✓':'FAIL'}</span><br><span class="amber">calibrările dinamice marcate separat rămân vizibile în rezultat</span>`;}
    else {const parts=[...gate.missing];if(!aa?.pass)parts.push('Player A stat audit');if(!ab?.pass)parts.push('Player B stat audit');$('gateReason').textContent='STRICT DATA GATE: '+parts.join(' · ');}
  }
  bind('a');bind('b');

  $('ruleCount').textContent=D.rules.rules.length+20;
  function openTab(id){document.querySelectorAll('.tabs button').forEach(x=>x.classList.toggle('active',x.dataset.tab===id));document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.id===id));}
  ...
  const rogue=CE.rogue60Baseline(CD.level60.undeadRogue,P),mp=D.pvpProfiles.mage_frost_gnome_p6_core,mageArmor=Math.trunc(mp.stats.armorBeforeTalents+mp.stats.intellect*0.5),barrier=Math.round(D.pvpSpellbooks.Mage.iceBarrier.absorbBase+mp.stats.spellDamage*D.pvpSpellbooks.Mage.iceBarrier.spCoeff),manaShield=D.pvpSpellbooks.Mage.manaShield.absorbBase+(mp.gearEffects?.manaShieldBonusAbsorb||0);
  ...
  function renderDuel(r){if(r.error){$('duelSummary').innerHTML=`<div class="red"><b>${r.error}</b>: ${r.missing.join(', ')}</div>`;return;}const winClass=r.winner==='Timeout'?'amber':'winner',cal=(r.calibrationRequired||[]).map(x=>`<li>${x}</li>`).join('');$('duelSummary').innerHTML=`<div class="${winClass}">WINNER: ${r.winner}</div><div class="result-meta">Durată ${r.duration}s · Rogue ${r.final.rogue.hp} HP · Mage ${r.final.mage.hp} HP / ${r.final.mage.mana} mana · range ${r.final.range} yd</div>${cal?`<div class="calibration"><b>Încă neincluse în rezultat:</b><ul>${cal}</ul></div>`:''}`;$('timeline').innerHTML=r.timeline.map(e=>`<div class="timeline-row ${e.kind}"><span>${e.t.toFixed(1)}s</span><b>${e.actor}</b><em>${e.text}</em></div>`).join('');$('quickResult').classList.remove('hidden');$('quickResult').innerHTML=`<span class="${winClass}">WINNER: ${r.winner}</span><span>${r.duration}s</span><button id="seeTimeline">Timeline</button>`;$('seeTimeline').onclick=()=>openTab('duel');}
  $('runBtn').onclick=()=>{const seed=(Number($('duelCode').value)||1337)>>>0,r=DUEL.run(seed,config());renderDuel(r);$('batchSummary').innerHTML='';openTab('duel');};
  function runBatch(button,count){const seed=(Number($('duelCode').value)||1337)>>>0;setBatchButtons(true);$('runBtn').disabled=true;const old=button.textContent;button.textContent='Simulez…';setTimeout(()=>{const b=DUEL.batch(seed,count,config());if(b.error)$('batchSummary').innerHTML=`<span class="red">${b.error}: ${b.missing.join(', ')}</span>`;else $('batchSummary').innerHTML=`<div class="batch-grid"><div><span>Duels</span><b>${b.count.toLocaleString('ro-RO')}</b></div><div><span>Rogue</span><b>${b.rates.Rogue}%</b></div><div><span>Mage</span><b>${b.rates.Mage}%</b></div><div><span>Timeout</span><b>${b.rates.Timeout}%</b></div><div><span>Avg</span><b>${b.avgDuration}s</b></div></div>`;button.textContent=old;updateGate();openTab('duel');},30);}
  $('batch1Btn').onclick=()=>runBatch($('batch1Btn'),1000);$('batchBtn').onclick=()=>runBatch($('batchBtn'),10000);$('batch100Btn').onclick=()=>runBatch($('batch100Btn'),100000);updateGate();
})();