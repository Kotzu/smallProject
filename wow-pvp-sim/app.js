(function(){
  ...
  function renderStatus(p){const prof=profileFor(state[p]);const base=(state[p].class==='Rogue'&&state[p].race==='Undead')||(state[p].class==='Mage'&&state[p].race==='Gnome');let txt=prof?'<span class="green">✓ gear static verificat</span>':'<span class="amber">gear exact încă neîncărcat</span>';if(base)txt+=' · <span class="green">base stats lvl60 verificate</span>';$(p+'Status').innerHTML=txt;updateGate();}
  function updateGate(){const pa=profileFor(state.a),pb=profileFor(state.b);const ready=E.profileReady(pa)&&E.profileReady(pb);$('runBtn').disabled=!ready;$('batchBtn').disabled=!ready;$('gateReason').textContent=ready?'Gata de simulat':'STRICT DATA GATE: lipsesc talent build/spellbook/gear complet pentru cel puțin un player.';}
  bind('a');bind('b');
  ...
  const mage=CE.gnomeMage60Naked(CD.level60.gnomeMage);
  ...
  $('mageCharacter').innerHTML=[['STR',mage.stats.str],['AGI',mage.stats.agi],['STA',mage.stats.sta],['INT după racial',mage.stats.int],['SPI',mage.stats.spi],['HP naked',mage.health],['Mana naked',mage.mana],['Armor naked',mage.armor]].map(x=>`<div class="stat"><span>${x[0]}</span><b>${x[1]}</b></div>`).join('');
