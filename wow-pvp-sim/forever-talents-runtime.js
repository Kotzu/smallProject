(function(){
  ...
  function nodesForTree(treeId){
    ...
    return Object.values(src).map(n=>({...n,maxRank:Array.isArray(n.ranks)?n.ranks.length:Object.keys(n.descriptions||{}).length})).sort((a,b)=>(a.row-b.row)||(a.col-b.col)||(a.id-b.id));
  }
  function tree(className,treeName){
    const norm=String(treeName||'').toLowerCase();
    const t=classTrees(className).find(x=>String(x.description||'').toLowerCase().replace(String(CLASS_PREFIX[className]||'').toLowerCase(),'')===norm);
    return t?nodesForTree(t.id):null;
  }
  function classInfo(className){
    const trees=classTrees(className);if(!trees.length)return null;
    const out={className,trees:{},treeIds:{}};
    for(const t of trees){const name=String(t.description).replace(CLASS_PREFIX[className],'');out.trees[name]=nodesForTree(t.id).length;out.treeIds[name]=t.id;}
    return out;
  }
  function classTotal(className){return classTrees(className).reduce((n,t)=>n+nodesForTree(t.id).length,0);}
  function allCoverage(){
    const classes=Object.keys(CLASS_PREFIX),perClass={},treeCount=[];let totalNodes=0,totalTrees=0;
  ...
  const api={
    ...
    classInfo,classTotal,tree,nodesForTree,
